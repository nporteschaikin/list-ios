...
- (void)categoriesControllerDidFetchCategories:(CategoriesController *)categoriesController {
    ...
    [self.headerView selectControlAtIndex:0];
    ...
    if (!self.postsTableViewController) {
        self.postsTableViewController = [[PostsTableViewController alloc] initWithPostsController:self.postsController session:self.session];
...
- (void)locationHeaderView:(LocationHeaderView *)locationHeaderView didSelectControlAtIndex:(NSInteger)index {
    ...
    NSArray *categories = categoriesController.categories;
    if (self.postsTableViewController && categories.count) {
        [self.postsController requestPosts];
    }
