...
@property (strong, nonatomic) LIconView *titleIconView;
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self.titleView insertSubview:self.titleIconView
...
- (void)layoutTitleView {
    ...
    x = CGRectGetMidX(self.titleRadarView.frame) - (CGRectGetWidth(self.titleIconView.frame) / 2);
    y = CGRectGetMidY(self.titleRadarView.frame) - (CGRectGetHeight(self.titleIconView.frame) / 2);
    w = CGRectGetWidth(self.titleIconView.frame);
    h = CGRectGetHeight(self.titleIconView.frame);
    self.titleIconView.frame = CGRectMake(x, y, w, h);
...
- (void)animateTitleView:(BOOL)animate {
    if (animate && !self.titleViewIsAnimating) {
        ...
        [self.titleIconView.layer removeAllAnimations];
...
- (LIconView *)titleIconView {
    if (!_titleIconView) {
        _titleIconView = [[LIconView alloc] init];
        _titleIconView.icon = LIconLocation;
        _titleIconView.iconSize = [UIFont list_headerViewFont].pointSize;
        _titleIconView.iconColor = [UIColor whiteColor];
